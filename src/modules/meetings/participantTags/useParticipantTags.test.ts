...
const socketHandler = mockRegisterTypeHandler.mock.calls.find(
  ...
  action: string
  payload: AllTagsPayload | TagChangedPayload
}) => void
...
test('"all" payload populates tags per user', () => {
  ...
  socketHandler({
    action: 'all',
    payload: { meeting: MEETING, tags: { 'group:a': [1, 2], 'group:b': [2] } }
  })
...
test('"changed" payload invalidates effects reading an existing meeting store', () => {
  ...
  socketHandler({
    action: 'all',
    payload: { meeting, tags: { 'group:a': [7] } }
  })
  ...
  socketHandler({
    action: 'changed',
    payload: { meeting, user: 7, tags: { group: ['b', 'c'] } }
  })
...
test('getUserTags filters on namespace', () => {
  ...
  socketHandler({
    action: 'changed',
    payload: { meeting, user: 3, tags: { group: ['a'], role: 'chair' } }
  })
