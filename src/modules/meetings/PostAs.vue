  ...
  setup(props, { emit }) {
    ...
    const value: Ref<Partial<Author>> = editingMode
      ...
      : computed({
          get() {
            return (
              postAsDefaults.get(meetingId.value) || {
    ...
    const postAs = computed<string | undefined>({
      get() {
        ...
        return meeting_group ? `group:${meeting_group}` : `user:${author}`
      },
      set(newValue) {
        if (!newValue) return
    ...
    function preserveCurrentAuthor(
      newOptions?: AutocompleteItem[]
    ): AutocompleteItem[] {
      newOptions = newOptions ?? []
      const { author } = value.value
      if (!author) return newOptions
      const finder = (item: AutocompleteItem) =>
        author === Number(item.value.split(':')[1])
      ...
      return preserve ? [preserve, ...newOptions] : newOptions
    }
    ...
    const options = computed(() => {
      const groups = isModerator.value ? meetingGroups : userGroups
