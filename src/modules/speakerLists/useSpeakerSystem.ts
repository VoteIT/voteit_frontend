...
const { getSystem, getSystemSpeakerLists, getList, getCurrent } = useSpeakerLists()
...
export default function useSpeakerSystem (systemId: Ref<number>, agendaItem?: Ref<number>) {
  ...
  const currentSpeakerQueue = computed(() => systemActiveList.value && systemActiveList.value.queue)
