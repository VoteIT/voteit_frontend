<template>
  <div>
        ...
        <template v-slot="{ close }">
          <v-form @submit.prevent="createSystem()">
            ...
            <v-select
              ...
              v-model="systemData.method_name"
              :items="orderMethods"
            />
            ...
            <v-expand-transition>
              <div v-if="createSystemSettings">
                <v-text-field
                  v-for="field in createSystemSettings"
                  :key="field.name"
                  v-bind="field"
                />
              </div>
            ...
            <v-text-field
              ...
              min="0"
              max="2"
              v-model="systemData.safe_positions"
            />
            <CheckboxMultipleSelect
              v-model="systemData.meeting_roles_to_speaker"
    ...
    <Widget
      class="speaker-system my-4"
      v-for="{ system, menu, data, userSearch } in systems"
      :key="system.pk"
    >
      <div class="d-flex">
      ...
      <template #default="{ close }">
        <v-form @submit.prevent="saveSystem()">
          ...
          <v-select
            required
            :label="t('speaker.systemMethod')"
            v-model="editSystemData.method_name"
            :items="orderMethods"
          />
          <!-- TODO Better dynamic forms -->
          <v-expand-transition>
            <div v-if="editSystemSettings">
              <v-text-field
                v-for="field in editSystemSettings"
                :key="field.name"
                v-bind="field"
              />
            </div>
          ...
          <v-text-field
            ...
            min="0"
            max="2"
            v-model="editSystemData.safe_positions"
          />
          <CheckboxMultipleSelect
            v-model="editSystemData.meeting_roles_to_speaker"
            :settings="{ options: roleLabels }"
            :label="t('speaker.speakerRoles')"
          />
          <div class="text-right">
...
const createSystemSettings = computed<Record<string, any>[] | undefined>(() =>
  getMethodSettings(systemData)
)
...
const createReady = computed(
  () =>
    !creating.value &&
    systemData.title &&
    systemData.method_name &&
    systemData.meeting_roles_to_speaker?.length
)
...
function getMethodSettings(data: Partial<SpeakerSystem>) {
  if (data.method_name === SpeakerSystemMethod.Priority) {
    // TODO BETTER
...
const editSystemReady = computed(
  () => editSystemData.title && editSystemData.meeting_roles_to_speaker?.length
)
const editSystemSettings = computed<Record<string, any>[] | undefined>(() =>
  getMethodSettings(editSystemData)
)
...
function getSystemData(
  ...
  return [
    ...
    },
    {
      ...
      value: system.meeting_roles_to_speaker
...
const systems = computed(() => {
  return allSpeakerSystems.value.map((system) => {
    ...
    return {
      ...
      userSearch: {
        ...
        filter: (user: IUser) => !userIds.includes(user.pk),
        onSubmit: (user: number) =>
          speakerSystemType.addRoles(system.pk, user, SpeakerSystemRole.Speaker)
      }
    }
  })
})
...
const orderMethods = computed(() => {
  return Object.values(SpeakerSystemMethod).map((name) => {
    return {
      value: name,
