...
function isActivePresenceComponent (meeting: Meeting | number) {
  const meetingId = typeof meeting === 'number'
    ? ref(meeting)
    : toRef(meeting, 'pk')
...
export function canAddPresenceCheck (meeting: Meeting | number): boolean {
  return !!isModerator(meeting) && isActiveMeeting(meeting) && isActivePresenceComponent(meeting)
}

export function canChangePresenceCheck (check: PresenceCheck): boolean {
  return !!isModerator(check.meeting) && isOpenPresenceCheck(check) && isActivePresenceComponent(check.meeting)
}
