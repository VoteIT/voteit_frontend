...
export default function useRules(t: ComposerTranslation) {
  function email(value: string) {
    return (
      !value.length || emailPattern.test(value) || t('invites.email.invalid')
    )
  }
  ...
  function max(max: number) {
    return (value: string) =>
      !value || Number(value) <= max || t('rules.maxValue', max)
  }
  ...
  function min(min: number) {
    return (value: string) =>
      !value || Number(value) >= min || t('rules.minValue', min)
  }
  ...
  function multiline(rule: Rule): Rule {
    return (value: string) => {
      for (const [i, line] of enumerate(value.split('\n'), 1)) {
        ...
        if (line && typeof result === 'string')
          return `${t('rules.multilineFailed', i)}: ${result}`
      }
      return true
    }
  }
  ...
  function swedishSSN(value: string) {
    return (
      !value.length ||
      swedishSSNPattern.test(value) ||
      t('invites.swedish_ssn.invalid')
    )
  }
  ...
  function tabSeparated(...rules: Rule[]): Rule {
    return (value: string) => {
      ...
      if (values.length !== rules.length)
        return t('rules.tabSeparatedBadLength', rules.length)
  ...
  function tabSeparatedEqualColumns(max: number, min: number = 1): Rule {
    return (value: string) => {
      ...
      if (max && columnCount > max)
        return t('rules.tabSeparatedMaxColumns', max)
      for (const [i, row] of enumerate(value.split('\n'), 1)) {
        if (!row.trim()) continue
        if (row.split('\t').length !== columnCount)
          return t('rules.tabSeparatedBadColumnCount', i)
      }
      return true
    }
  }
